...
"key": {
    ...
    "status": "PENDING"
        ...
        const reply = (teks) => {
            hexa.sendMessage(from, teks, text, {quoted:fgi})
        }
...
const daftar3 = [{buttonId: `.verify`,buttonText: {displayText: `ꪶ𖣂 VERIFY 𖣂ꫂ`,},type: 1,},]
...
*メLord Akira⁴̅⁰͍⁴̵ || ${_registered.length} User KirBotz×*
...
*「 AkiraBotz 」*`
         kirbotz = await getBuffer(ppimg)
             buttons = [{buttonId: `.menu`,buttonText:{displayText: `✗⃝📘MENU`},type:1}]
              imageMsg = (await hexa.prepareMessageMedia(kirbotz, "imageMessage", { thumbnail: kirbotz, })).imageMessage
              buttonsMessage = {footerText:'_Pencet Dulu *RULES*_\n_Sebelum Pencet *MENU*_', imageMessage: imageMsg,
              contentText:`${anuu}`,buttons,headerType:4}
              prep = await hexa.prepareMessageFromContent(from,{buttonsMessage},{quoted: floc})
...
case 'bc':
             ...
             bc1000 = await hexa.downloadMediaMessage(encmedia)

             for (let _ of anu1000) {

             hexa.sendMessage(_.jid, bc1000, image, {quoted: mek, caption: `*「 KAHFZ GAMTENG 」*\n\n${body.slice(4)}`})
...
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: floc})
		            ...
		            let media2 = await hexa.downloadAndSaveMediaMessage(encmedia2)
		            ran = getRandom('.mp4')
...
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: floc})
		            ...
		            let owgi = await hexa.downloadAndSaveMediaMessage(ger)
